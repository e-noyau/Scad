...
screw_length=clamp_length+clamp_thickness-13-(rounding*4)-screw_length_reduction-offset;
    ...
    difference() {
        minkowski() {
          union(){
            ...
            // side flat
            translate([0,-tr,0])
              cube([length-ti*2,thickness,width]);
            // bottom corner
            translate([length-ti*2,0,0]) rotate([0,0,90])
              s_corner(tr);
            // bottom flat
            translate([length-ti,0,0]) cube([thickness,depth,width]);
            // offset
            if (offset) {
              translate([length-ti-offset, hole_shift-screw_diameter, 0])
                cube([offset, depth-hole_shift+screw_diameter ,width]);
            }
          }
          sphere(r=rounding,$fn=roundedness);
      }
      translate([0.1-clamp_thickness*0.8+rounding*2+(rounding*0.6), hole_shift, width/2])      
        rotate([0,-90,0]) {
           screw_thread(screw_diameter+((bolt_hole_space_factor/100)*screw_diameter),
                        4, 55, clamp_thickness+0.2, PI/2,2);
           translate ([0,0,0.2])
             hex_countersink_ends(1.5,screw_diameter,55,2,clamp_thickness-0.2);
      }
      translate([length/2-ti, thickness/2-tr, -width/2]) cylinder(h=width*2,d=3,$fn=30);
   }
}
